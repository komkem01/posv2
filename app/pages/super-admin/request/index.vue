<template>
  <div class="min-h-screen bg-slate-50 p-4 md:p-6">
    <div
      class="max-w-6xl mx-auto bg-white border border-slate-200 rounded-2xl p-4 md:p-6 shadow-sm"
    >
      <div
      ...
      >
        <div>
          ...
          <p class="text-xs text-slate-500">
            กดอนุมัติ/ปฏิเสธคำขอ และระบบจะส่งอีเมลให้อัตโนมัติ
          </p>
        </div>
        <div class="flex items-center justify-end gap-2">
          <button
            @click="loadRequests"
            class="px-3 py-2 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700"
          >
            รีเฟรช
          </button>
          <button
            @click="openLogoutModal"
            class="px-3 py-2 rounded-xl text-xs font-bold bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200"
          >
            ออกจากระบบ
          </button>
        </div>
              ...
              <td class="py-3 text-right">
                ...
                >
                  ...
                  <button
                    ...
                    class="px-2.5 py-1.5 rounded-lg text-white bg-rose-600 hover:bg-rose-700 font-bold"
                  >
                    ปฏิเสธ
                  </button>
                </div>
    ...
    <Transition name="fade">
      ...
      >
        ...
        ></div>
        <div
          class="relative w-full max-w-lg bg-white rounded-2xl p-5 shadow-xl border border-slate-200"
        >
          <h2 class="text-lg font-extrabold text-slate-800">อนุมัติสำเร็จ</h2>
          <p class="text-xs text-slate-500 mt-1">
            ส่งข้อมูลด้านล่างให้ลูกค้า (และระบบพยายามส่งอีเมลอัตโนมัติแล้ว)
          </p>
          <div
            class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm space-y-2"
          >
            <p>
              <span class="font-bold">อีเมล:</span> {{ credential.owner_email }}
            </p>
            <p>
            ...
            <p>
              <span class="font-bold">รหัสผ่านชั่วคราว:</span>
              {{ credential.temp_password }}
            </p>
            <p>
              <span class="font-bold">ส่งอีเมล:</span>
              {{
                credential.email_sent ? "สำเร็จ" : "ไม่สำเร็จ/ไม่ตั้งค่า SMTP"
              }}
            </p>
            <p v-if="credential.email_error" class="text-rose-600">
              <span class="font-bold">รายละเอียด:</span>
              {{ credential.email_error }}
            </p>
          </div>
          <button
            @click="showCredentialModal = false"
            class="mt-4 w-full px-3 py-2.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700"
          >
            ปิดหน้าต่าง
          </button>
        </div>
      ...
      >
        ...
        >
          ...
          <div class="mt-4 flex gap-2">
            ...
            <button
              @click="showRejectModal = false"
              class="px-4 py-2.5 rounded-xl text-sm font-bold bg-slate-100 hover:bg-slate-200 text-slate-700"
            >
              ยกเลิก
            </button>
          </div>
    ...
    <AppModal ref="logoutModalRef" :show-accent="false">
      <div class="flex items-start gap-3">
        <div
          class="shrink-0 mt-0.5 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-rose-50 text-rose-600 border border-rose-100"
        >
          <svg
            class="h-4.5 w-4.5"
            fill="none"
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500 leading-relaxed">
            หากออกจากระบบตอนนี้
            คุณจะต้องเข้าสู่ระบบใหม่อีกครั้งเพื่อจัดการคำขอร้านค้า
          </p>
        </div>
      </div>
      ...
      <div class="mt-5 flex flex-col-reverse sm:flex-row gap-2">
        <button
          ...
          class="px-4 py-2.5 rounded-xl text-sm font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors"
        >
          ยกเลิก
        </button>
        <button
          @click="handleLogout"
          class="flex-1 px-3 py-2.5 rounded-xl text-sm font-semibold text-white bg-rose-600 hover:bg-rose-700 active:bg-rose-800 transition-colors"
        >
          ยืนยันออกจากระบบ
        </button>
      </div>
...
const loadRequests = async () => {
  ...
  try {
    const res = await get<StoreRequest[]>(
      "/api/v1/system/store-registration-request",
      { size: 100, sort_by: "created_at", order_by: "desc" },
    );
...
const approve = async (item: StoreRequest) => {
  try {
    const res = await post<ApprovePayload>(
      `/api/v1/system/store-registration-request/${item.id}/approve`,
      { review_note: "" },
    );
...
const reject = async () => {
  ...
  try {
    await post(
      `/api/v1/system/store-registration-request/${rejectTarget.value.id}/reject`,
